...
export default {
	...
	onReachBottom() {
		...
		if(_this.onReachBottom){
			...
			wx.pro.request({
				url: `${configs.catShop.apiBaseUrl}exa-${configs.Store.ExtensionApp}/proxy?url=` + encodeURIComponent(configs.Store.url + 'assistant/sv-'+configs.Store.Service+'/bookings/of-assistant?take='+_this.pageSize+'&skip='+_this.page),
				method: 'GET',
				header: {
	...
	methods: {
		...
		getdata() {
			...
			var _this = this;
			wx.pro.request({
				url: `${configs.catShop.apiBaseUrl}exa-${configs.Store.ExtensionApp}/proxy?url=` + encodeURIComponent(configs.Store.url + 'assistant/sv-'+configs.Store.Service+'/bookings/of-assistant?take=15'),
				method: 'GET',
				header: {
					token:Auth.proxy.token.token
				}
